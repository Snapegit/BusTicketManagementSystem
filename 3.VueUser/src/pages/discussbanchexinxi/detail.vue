...
<div>
	<div :style='{"border":"1px solid #D8D8D8","padding":"16px 20px 10px","margin":"65px auto 20px","borderRadius":"0","background":"transparent","borderWidth":"0 0 3px","width":"68%"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'》'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/discussbanchexinxi"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"width":"58%","padding":"0","margin":"0 0 10px 2%","background":"#fff","display":"flex","order":"2"}'>
			...
			<div class="info" :style='{"minHeight":"520px","padding":"10px 0 0","margin":"0 0 0 0","background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"4px 10px","margin":"0 0 6px 0","background":"transparent","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>关联表id</div>
					<div  :style='{"padding":"0 10px","fontSize":"15px","lineHeight":"40px","color":"#000","flex":"1","height":"40px"}'>{{detail.refid}}</div>
				</div>
				<div class="item" :style='{"padding":"4px 10px","margin":"0 0 6px 0","background":"transparent","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>用户名</div>
					<div  :style='{"padding":"0 10px","fontSize":"15px","lineHeight":"40px","color":"#000","flex":"1","height":"40px"}'>{{detail.nickname}}</div>
				</div>
				<div class="item" :style='{"padding":"4px 10px","margin":"0 0 6px 0","background":"transparent","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>评论内容</div>
					<div  :style='{"padding":"0 10px","fontSize":"15px","lineHeight":"40px","color":"#000","flex":"1","height":"40px"}'>{{detail.content}}</div>
				</div>
				<div class="item" :style='{"padding":"4px 10px","margin":"0 0 6px 0","background":"transparent","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>回复内容</div>
					<div  :style='{"padding":"0 10px","fontSize":"15px","lineHeight":"40px","color":"#000","flex":"1","height":"40px"}'>{{detail.reply}}</div>
				</div>
				<div class="btn" :style='{"padding":"10px 20px","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"5px","background":"#000","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('discussbanchexinxi','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","color":"#fff","borderRadius":"5px","background":"#000","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('discussbanchexinxi','删除')" @click="delClick">删除</el-button>
					<!-- hasChat $hasChat -->
				</div>
			</div>
		</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDiscussList(page) {
        this.$http.get('discussdiscussbanchexinxi/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discussdiscussbanchexinxi/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussdiscussbanchexinxi/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
	  ...
	  addDiscussNum(type){
		  ...
		  this.$http.post('discussbanchexinxi/update',this.detail).then(res=>{
		...
		// 修改
		editClick(){
			this.$router.push(`/index/discussbanchexinxiAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此班车信息评论表？')
			  .then(_ => {
			    this.$http.post('discussbanchexinxi/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
