...
<div>
	<div :style='{"border":"1px solid #D8D8D8","padding":"16px 20px 10px","margin":"65px auto 20px","borderRadius":"0","background":"transparent","borderWidth":"0 0 3px","width":"68%"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'》'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/bancishijian"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"width":"58%","padding":"0","margin":"0 0 10px 2%","background":"#fff","display":"flex","order":"2"}'>
			...
			<div class="info" :style='{"minHeight":"520px","padding":"10px 0 0","margin":"0 0 0 0","background":"#fff","flex":"1"}'>
				...
				<div class="btn" :style='{"padding":"10px 20px","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"5px","background":"#000","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('bancishijian','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","color":"#fff","borderRadius":"5px","background":"#000","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('bancishijian','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/bancishijianAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此班次时间？')
			  .then(_ => {
			    this.$http.post('bancishijian/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
