...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","top":"130px","flexWrap":"wrap","left":"0","background":"transparent","display":"flex","width":"20%","position":"absolute","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","color":"#000","display":"inline-block"}'>班次时间：</div>
        <el-input v-model="formSearch.bancishijian" placeholder="班次时间" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('bancishijian','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"20px 10px 0 0","outline":"none","color":"#fff","borderRadius":"60px","background":"#313131","width":"auto","fontSize":"15px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/bancishijianAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"16px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.bancishijian != '') searchWhere.bancishijian = '%' + this.formSearch.bancishijian + '%';
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`bancishijian/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/bancishijianDetail', query: params});
      },
