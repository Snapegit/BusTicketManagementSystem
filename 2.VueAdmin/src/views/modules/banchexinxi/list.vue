			...
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">班车车号</label>
						<el-input v-model="searchForm.banchechehao" placeholder="班车车号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">始发站点</label>
						<el-input v-model="searchForm.shifazhandian" placeholder="始发站点" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">终点站点</label>
						<el-input v-model="searchForm.zhongdianzhandian" placeholder="终点站点" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">司机姓名</label>
						<el-input v-model="searchForm.sijixingming" placeholder="司机姓名" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('banchexinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('banchexinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='false' :sortable='true'  
						...
						label="班次时间">
						<template slot-scope="scope">
							{{scope.row.bancishijian}}
						</template>
					</el-table-column>
					<!-- 无 -->
					<el-table-column :resizable='false' :sortable='true' prop="cheliangzhaopian" width="200" label="车辆照片">
						<template slot-scope="scope">
					...
					<el-table-column :resizable='false' :sortable='true'  
						...
						label="始发站点">
						<template slot-scope="scope">
							{{scope.row.shifazhandian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='false' :sortable='true'  
						prop="zhongdianzhandian"
						label="终点站点">
						<template slot-scope="scope">
							{{scope.row.zhongdianzhandian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='false' :sortable='true'  
						prop="sijixingming"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('banchexinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont " :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('banchexinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont " :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('banchexinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont " :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('banchexinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont " :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								删除
							</el-button>
						</template>
...
import AddOrUpdate from "./add-or-update";
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.banchechehao!='' && this.searchForm.banchechehao!=undefined){
            params['banchechehao'] = '%' + this.searchForm.banchechehao + '%'
          }
           if(this.searchForm.shifazhandian!='' && this.searchForm.shifazhandian!=undefined){
            params['shifazhandian'] = '%' + this.searchForm.shifazhandian + '%'
          }
           if(this.searchForm.zhongdianzhandian!='' && this.searchForm.zhongdianzhandian!=undefined){
            params['zhongdianzhandian'] = '%' + this.searchForm.zhongdianzhandian + '%'
          }
           if(this.searchForm.sijixingming!='' && this.searchForm.sijixingming!=undefined){
            params['sijixingming'] = '%' + this.searchForm.sijixingming + '%'
          }
	...
	this.$router.push({path:'/discussbanchexinxi',query:{refid:id}});
    },
    // 下载
    download(file){
	...
	banchexinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'banchexinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
