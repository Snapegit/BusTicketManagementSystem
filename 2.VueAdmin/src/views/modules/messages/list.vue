		...
		<template v-if="showFlag">
			...
			<div :style='{"width":"100%","padding":"10px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='false' :sortable='true' prop="content" label="留言内容">
						<template slot-scope="scope">
                            <span v-html="scope.row.content"></span>
						</template>
					</el-table-column>
					<!-- 无 -->
					<el-table-column :resizable='false' :sortable='true' prop="cpicture" width="200" label="留言图片">
						<template slot-scope="scope">
							<div v-if="scope.row.cpicture">
								<img v-if="scope.row.cpicture.substring(0,4)=='http'" :src="scope.row.cpicture.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.cpicture.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<!-- 无 -->
					<el-table-column :resizable='false' :sortable='true' prop="rpicture" width="200" label="回复图片">
						<template slot-scope="scope">
							<div v-if="scope.row.rpicture">
								<img v-if="scope.row.rpicture.substring(0,4)=='http'" :src="scope.row.rpicture.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.rpicture.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
